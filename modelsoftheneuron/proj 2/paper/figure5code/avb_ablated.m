...
ava_sw = [0, 4]; %weights for synapses
...
%% AVB Interneuron ( 3 , 4 )
ydot(3) = 0; ydot(4) = 0; y(3) = 0; y(4) = 0;

%% DB Motorneuron ( 5 , 6 )
db_gaps = [y(1),y(3),y(9)]; %neurons connected via gap jxns
db_gw = [2,0,3]; %weights for gaps
...
vb_gw = [0,3,1]; %weights for gaps
vb_syns = [y(4), y(12)]; %neurons connected via synapses
vb_sw = [0,3]; %weights for synapses
...
vd_sw = [2,0,70,2,13]; %weights for synapses
