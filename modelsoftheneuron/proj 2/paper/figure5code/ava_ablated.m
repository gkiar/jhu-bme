...
db_gw = [0,14,3]; %weights for gaps
db_syns = [y(2)]; %neurons connected via synapses
db_sw = [0]; %weights for synapses
...
vb_gw = [14,3,1]; %weights for gaps
vb_syns = [y(4), y(12)]; %neurons connected via synapses
vb_sw = [1,3]; %weights for synapses
vb_pols = [Ej(2), Ej(2)]; %excitatory(1) or inhibitory(2) synapses
...
vd_sw = [0,1,70,2,13]; %weights for synapses
