...
%% Neuron 1
Ig(1) = Gs * v(2) * (0 + (v(1)-Ej(1)) + 0); % Ej(1)or(2) depending on synapse + terms for each synapse
Ig(1) = 0; % replace with gap currents for connected neurons

Is(1) = Gg * (0 + (v(1)-v(1,1)) + 0); %Assuming 1 has a synapse onto 1
Is(1) = 0; % replace with synaptic currents for connected neurons

if t >= Iext(1) && t <= Iext(2)
    vdot(1) = (-Gc * (v(1) - Ec) - Ig(1) - Is(1) + Iext(3)) / C; % dV/dt
else
    vdot(1) = (-Gc * (v(1) - Ec) - Ig(1) - Is(1)) / C; % dV/dt
end
vdot(2) = arr * phi(B, vth, v(1)) * (1-v(2)) - ad * v(2); % ds/dt
